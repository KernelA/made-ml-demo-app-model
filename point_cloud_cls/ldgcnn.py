...
class SimpleClsLDGCN(nn.Module):
    ...
    def __init__(self, in_chan: int, out_chan: int, num_neighborhoods: int = 16) -> None:
        assert num_neighborhoods // 2 > 1
        super().__init__()
        out_conv1_chan = 16
        self.conv1 = gnn.DynamicEdgeConv(MLP((2 * in_chan, 32, out_conv1_chan)), k=num_neighborhoods)
        out_conv2_chan = 64
